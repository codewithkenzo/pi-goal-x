...
Second fix invariant: post-stop blocking is turn-scoped by a lightweight generation counter. `turn_start`, `session_start`, `session_tree`, and `before_agent_start` advance the generation; post-stop markers block only when their stored generation matches the current one. Older markers self-clear on the next `tool_call`, preventing prior turn/session poisoning when hook order skips `turn_start`. Explicit `/goal-resume` already-running and `/goal-focus` active continuation paths also clear stale marker state before arming continuation. Non-progress prep/recon tools during active checkpoints do not mark the goal stopped; they only leave `goalWorkToolCalledThisTurn=false`, so `turn_end` skips auto-continue if no meaningful progress tool ran.
